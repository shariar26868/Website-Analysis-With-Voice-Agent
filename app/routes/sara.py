...
# from app.config import get_db
# from app.services.sara_agent import initiate_sara_call
# from app.utils import logger, normalize_phone
...
#         # Normalize to E.164 format (Bangladesh: +880)
#         phone_number = normalize_phone(phone_number, country_code="880")
...
async def request_sara_call(request: SaraCallRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not ObjectId.is_valid(request.lead_id):
        ...
        if not lead:
            raise HTTPException(status_code=404, detail=f"Lead not found with ID: {request.lead_id}")
        ...
        try:
            ...
            # If normalization fails, use original number
            pass
        ...
        return SaraCallResponse(
            success=True,
            message=f"Sara will call {phone_number} shortly",
            call_id=request.lead_id,
            call_status=CallStatus.IN_PROGRESS
        )
...
async def get_call_status(lead_id: str):
    ...
    try:
        ...
        # Convert ObjectId to string if call log exists
        if call_log:
